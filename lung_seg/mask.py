...
def apply(image, model=None, force_cpu=False, batch_size=1, volume_postprocessing=True, noHU=False):
    if model is None:
    ...
    tvolslices = np.expand_dims(tvolslices,1)
    with torch.no_grad():
        # for X in tqdm(dataloader_val):
        for slice in range (tvolslices.shape[0]//batch_size):
            X = torch.FloatTensor(tvolslices[slice*batch_size : (slice+1)*batch_size]).to(device)
            # X = X.float().to(device)
            prediction = model(X)
            pls = torch.max(prediction, 1)[1].detach().cpu().numpy().astype(np.uint8)
            timage_res = np.vstack((timage_res, pls))

        slice = tvolslices.shape[0]//batch_size
        if slice*batch_size != tvolslices.shape[0]:
            X = torch.FloatTensor(tvolslices[slice*batch_size : tvolslices.shape[0]]).to(device)
            X = X.float().to(device)
            prediction = model(X)
            pls = torch.max(prediction, 1)[1].detach().cpu().numpy().astype(np.uint8)
            timage_res = np.vstack((timage_res, pls))
...
def get_model(modeltype, modelname, n_classes=3):
    if n_classes==3:
        state_dict = torch.load('./lung_seg/weight_lung.pth')
    elif n_classes==6:
        state_dict = torch.load('./lung_seg/weight_lunglobes.pth')
        # state_dict = torch.load('/home/avitech-pc4/Nam/lungmask/checkpoint/resunet/resunet_temp_100.pth')
        # state_dict = torch.load('/home/avitech-pc4/Nam/lungmask/checkpoint/unet/unet_temp_100.pth')
        # state_dict = torch.load('./lung_seg/net50-0.004-0.006_LUNA.pth')
...
def apply_fused(image, basemodel = 'LTRCLobes', fillmodel = 'R231', force_cpu=False, batch_size=1, volume_postprocessing=True, noHU=False):
    '''Will apply basemodel and use fillmodel to mitiage false negatives'''
    mdl_r = get_model('unet',fillmodel, n_classes=3)
    mdl_l = get_model('unet',basemodel, n_classes=6)
