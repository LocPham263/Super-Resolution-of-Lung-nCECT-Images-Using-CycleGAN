...
#         image_name = '%s_%s.tiff' % (parsname, label)
...
class Visualizer():
    ...
    def display_current_results(self, visuals, epoch, save_result):
            ...
            if ncols > 0:
                ...
                for label, image in visuals.items():
                    ...
                    if idx % ncols == 0:
                        ...
                        label_html_row = ''
                white_image = np.ones_like(image_numpy.transpose([2, 0, 1])) * 1024
                while idx % ncols != 0:
        ...
        if self.use_html and (save_result or not self.saved):  # save images to a html file
            self.saved = True
            for label, image in visuals.items():
                # print(label, torch.max(image), torch.min(image), image.shape)
                image_numpy = util.tensor2im(image, self.scale_range)
                img_path = os.path.join(self.img_dir, 'epoch%.3d_%s.nii.gz' % (epoch, label))
                # print(image_numpy[0,:,:].shape)
                # cv2.imwrite(img_path, image_numpy[0,:,:])

                sitk.WriteImage(sitk.GetImageFromArray(image_numpy[0,:,:]), img_path)
