...
def postrocessing(label_image, spare=[]):
    ...
    for r in regions:
        if r.area > origlabels_maxsub[np.int(r.max_intensity)]:
            origlabels_maxsub[np.int(r.max_intensity)] = r.area
            region_to_lobemap[r.label] = r.max_intensity

    for r in regions:
        if (r.area < origlabels_maxsub[np.int(r.max_intensity)] or r.max_intensity in spare) and r.area>2: # area>2 improves runtime because small areas 1 and 2 voxel will be ignored
            ...
            if regions[regionlabels.index(mapto)].area == origlabels_maxsub[np.int(
                regions[regionlabels.index(mapto)].max_intensity)]:
                origlabels_maxsub[np.int(regions[regionlabels.index(mapto)].max_intensity)] += myarea
